...
for(d=f=0,g=a.length;g>f;d=++f)if(e=a[d],e===c)return d;return-1},d.g=function(a){return this.setMap(a)},d.g.prototype=new h.OverlayView,d.g.prototype.draw=function(){},d}())}).call(this)}.call(this);
	...
	var map = null, markers = [], markerCluster = null, address = null;
	 ...
	 */
	function initMap() {
		$('.wps-advanced_map').each(function(){

			var markers = [], markerCluster = null, address = null;

			var $this = $(this),
				id = $this.find('.wps-google-map').attr('id'),
				data = $this.data(),
				center = $.trim(data.center.split('/')),
				zoom = data.zoom,
				marker_image = data.marker_image,
				addresses = $.trim(data.addresses),
				style = data.style;
				fit = data.fit;
				search = data.search;
				search_zoom = data.search_zoom;
			map_options.map_style = wps_map_style(style);
			...
			if (map_options.map_style !== undefined && map_options.map_style !== '') {
			...
			jQuery.each(addresses, function( key, address ){
				...
				MapData.push({
					latLng: new google.maps.LatLng(address[0], address[1]),
				});
			});
			MapData['icon'] = marker_image;
				...
				geocoder.geocode({
						...
						if(response !== undefined && response !== null) {
							if(response[i] !== undefined) {
								place = response[i].formatted_address;
							}
						}
				});
				return function () {
					infowindow.setContent('<div class="wps-infowindow">'+place+'</div>');
	...
	function map_controls() {
		...
		// Zoom In
		google.maps.event.addDomListener(document.getElementById('wps-zoom-in'), 'click', function (e) {
			e.preventDefault();
		...
		// Zoom Out
		google.maps.event.addDomListener(document.getElementById('wps-zoom-out'), 'click', function (e) {
			e.preventDefault();
			var currentZoom = map.getZoom();
			currentZoom--;
			if (currentZoom > 19) {
		...
		// Map Type: Roadmap
		google.maps.event.addDomListener(document.getElementById('wps-map-type-roadmap'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Satellite
		google.maps.event.addDomListener(document.getElementById('wps-map-type-satellite'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Hybrid
		google.maps.event.addDomListener(document.getElementById('wps-map-type-hybrid'), 'click', function (e) {
			e.preventDefault();
		...
		// Map Type: Terrain
		google.maps.event.addDomListener(document.getElementById('wps-map-type-terrain'), 'click', function (e) {
			e.preventDefault();
		...
		if( search == 'yes' ) {
			jQuery("#wps-map-address").geocomplete();
			document.getElementById('wps-map-submit').addEventListener('click', function() {
				geocodeAddress( geocoder, map );
			});

			function geocodeAddress(geocoder, resultsMap) {
				var address = document.getElementById('wps-map-address').value;
				geocoder.geocode({'address': address}, function(results, status) {
				if (status === 'OK') {
					resultsMap.setCenter(results[0].geometry.location);

					resultsMap.setZoom(parseInt(search_zoom));
					} else {
		...
		jQuery('#wps-current-location').click(function (e) {
			e.preventDefault();
			...
			if (!jQuery('#wps-current-location').hasClass('draw')) {
				// Create Loading Element
				var loading = document.createElement('div');
				loading.setAttribute('class', 'wps-loader-container');
				loading.innerHTML = '<div class="svg-loader"></div>';
				document.getElementById($('.wps-google-map').attr('id')).appendChild(loading);
			}
			...
			if (navigator.geolocation) {
				...
				navigator.geolocation.getCurrentPosition(function (current) {
					...
					jQuery('#wps-current-location').addClass('draw');
					// Toggle Crrent Location Icon & Circle
					if (jQuery('#wps-current-location').hasClass('active')) {
	...
	function wps_map_style(map_style) {
		switch (map_style) {
			case 'ultra-light':
			...
			break;
			case 'light-gray':
				map_style = "[{\"featureType\":\"landscape\",\"stylers\":[{\"hue\":\"#FFBB00\"},{\"saturation\":43.4},{\"lightness\":37.6},{\"gamma\":1}]},{\"featureType\":\"road.highway\",\"stylers\":[{\"hue\":\"#FFC200\"},{\"saturation\":-61.8},{\"lightness\":45.6},{\"gamma\":1}]},{\"featureType\":\"road.arterial\",\"stylers\":[{\"hue\":\"#FF0300\"},{\"saturation\":-100},{\"lightness\":51.2},{\"gamma\":1}]},{\"featureType\":\"road.local\",\"stylers\":[{\"hue\":\"#FF0300\"},{\"saturation\":-100},{\"lightness\":52},{\"gamma\":1}]},{\"featureType\":\"water\",\"stylers\":[{\"hue\":\"#0078FF\"},{\"saturation\":-13.2},{\"lightness\":2.4},{\"gamma\":1}]},{\"featureType\":\"poi\",\"stylers\":[{\"hue\":\"#00FF6A\"},{\"saturation\":-1.0989010989},{\"lightness\":11.2},{\"gamma\":1}]}]";
			break;
			case 'uber-original':
			...
			break;
			case 'no-label':
				map_style = '[{"featureType":"all","elementType":"all","stylers":[{"visibility":"on"}]},{"featureType":"all","elementType":"geometry","stylers":[{"visibility":"on"}]},{"featureType":"all","elementType":"labels.text","stylers":[{"visibility":"off"}]},{"featureType":"all","elementType":"labels.text.fill","stylers":[{"saturation":36},{"color":"#000000"},{"lightness":40},{"visibility":"off"}]},{"featureType":"all","elementType":"labels.text.stroke","stylers":[{"visibility":"on"},{"color":"#000000"},{"lightness":16}]},{"featureType":"all","elementType":"labels.icon","stylers":[{"visibility":"off"}]},{"featureType":"administrative","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"administrative","elementType":"geometry.fill","stylers":[{"color":"#000000"},{"lightness":20}]},{"featureType":"administrative","elementType":"geometry.stroke","stylers":[{"color":"#000000"},{"lightness":17},{"weight":1.2}]},{"featureType":"landscape","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"landscape","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":20}]},{"featureType":"poi","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"poi","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":21}]},{"featureType":"road.highway","elementType":"geometry.fill","stylers":[{"color":"#ffffff"},{"lightness":17}]},{"featureType":"road.highway","elementType":"geometry.stroke","stylers":[{"color":"#000000"},{"lightness":29},{"weight":0.2},{"visibility":"off"}]},{"featureType":"road.highway","elementType":"labels.text.fill","stylers":[{"color":"#ffffff"}]},{"featureType":"road.highway","elementType":"labels.text.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.highway.controlled_access","elementType":"geometry.fill","stylers":[{"visibility":"on"},{"color":"#ffffff"}]},{"featureType":"road.highway.controlled_access","elementType":"geometry.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.arterial","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":18}]},{"featureType":"road.arterial","elementType":"geometry.fill","stylers":[{"color":"#ffffff"}]},{"featureType":"road.arterial","elementType":"geometry.stroke","stylers":[{"color":"#ffffff"}]},{"featureType":"road.arterial","elementType":"labels.text","stylers":[{"color":"#ff0000"}]},{"featureType":"road.arterial","elementType":"labels.text.fill","stylers":[{"color":"#000000"}]},{"featureType":"road.arterial","elementType":"labels.text.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.local","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":16}]},{"featureType":"road.local","elementType":"geometry.fill","stylers":[{"color":"#ffffff"},{"visibility":"on"}]},{"featureType":"road.local","elementType":"geometry.stroke","stylers":[{"visibility":"off"}]},{"featureType":"road.local","elementType":"labels.text","stylers":[{"color":"#ffffff"},{"visibility":"off"}]},{"featureType":"road.local","elementType":"labels.text.fill","stylers":[{"visibility":"off"},{"color":"#000000"}]},{"featureType":"transit","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"transit","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":19}]},{"featureType":"water","elementType":"all","stylers":[{"visibility":"off"}]},{"featureType":"water","elementType":"geometry","stylers":[{"color":"#000000"},{"lightness":17}]}]';
			break; 
   			default:
   				map_style = '';
		}
